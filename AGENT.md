...
| 项目在 git 仓库内（§7.1 前置） | `git -C <项目目录> rev-parse --is-inside-work-tree` | **必须**——自修靠 `git reset` 回滚。不在仓库内就引导人类 `git init`（向导会主动问） |
...
5. 检查 file logging 与 git 前置，缺 git 仓库时主动问要不要 `git init`；
